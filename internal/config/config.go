...
type Config struct {
	...
	Firebase struct {
		// ServerKey is the FCM legacy server key for sending push notifications.
		// Leave empty to disable push notifications (no-op).
		ServerKey string
	}
	...
	SMTP struct {
		Host     string
		Port     int
		Username string // auth username — may differ from From (e.g. Mailtrap)
		From     string
		Password string
	}
}
...
func LoadFromEnv() (Config, error) {
	...
	// SMTP_USERNAME defaults to SMTP_FROM for standard servers; set explicitly for Mailtrap / SendGrid.
	cfg.SMTP.Username = getenvDefault("SMTP_USERNAME", cfg.SMTP.From)
